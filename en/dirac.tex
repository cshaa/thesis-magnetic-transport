...
As stated in chapter 9.2 of \citet{GradshteynRyzhik}, the solutions to \eqref{eqn-parabolic-cylinder-ode} can be expressed as a linear combination of the functions
\begin{equation}
    D_\nu(w) \: , \;\;
    D_\nu(-w) \: , \;\;
    D_{-\nu-1}(\i w) \: , \;\;
...
\end{gather*}
We substitute using the equality $\dd{}{w} D_\nu(w) = \frac{w}{2} \, D_\nu(w) - D_{\nu+1}(w)$ from \citet{GradshteynRyzhik} and arrive at the equation:
\begin{align*}
    \begin{pmatrix}
        D_\nu(w_0) & -D_\nu(-w_0) \\[5pt]
...
For each fixed $\alpha$, the relation \eqref{eqn-dirac-epsilon-F-relation} defines a set of non-intersecting analytic functions $\big\{ \epsilon_0(p), \epsilon_1(p), ... \big\}$ defined on the entire $\R$. This can be demonstrated either from the properties of $F$ via the implicit mapping theorem (listed as 8.6 in \citet{KaupKaup1983}), or from the properties of $\Hf_\alpha(p)$ via Theorem \ref{thm-eigenval-holo}. We choose the latter approach:
\begin{equation*}
    \Hf_\alpha(p)
