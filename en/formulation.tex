...
\begin{defn}[Potential perturbation]
    ...
    Let $\Hilb = L^2(\R^2)$, $\Domain(H)$ a dense subset of $\Hilb$, $b>0$ and $V \in L^1_{\text{loc}}(\R)$. A self-adjoint operator $H: \Domain(H) \to \Hilb$ given by the equation
    \begin{equation*}
        H
        = P^2_x
        + \big( P_y + b \, Q_x \big)^2
...
\begin{defn}[Geometric perturbation, transl. inv. layer]
    ...
    \end{align*}
    defined on functions $\psi \in C^\infty(\Omega)$, such that $\psi(x,y,z) = 0$ on the boundary of $\Omega$. The~operator $H$ which is the closure of $-\Delta^\Omega_{\mathrm{D}, A}$ is called the \textbf{Landau Hamiltonian with a geometric perturbation}. We will investigate, which choices of~$\omega$ lead to $\SpE(H) \neq \emptyset$.
\end{defn}
...
\section{A refresher on linear operators}
